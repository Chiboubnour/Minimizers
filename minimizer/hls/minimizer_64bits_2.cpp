...
#pragma HLS UNROLL
        ...
        {
#pragma HLS UNROLL
            const ap_uint<SMER_BITS> fwd = win.range(
                2*(PAR_FACTOR-1-i)+2*SMER_SIZE-1, 2*(PAR_FACTOR-1-i));
            ap_uint<SMER_BITS> rev = 0;
            for (int t = 0; t < SMER_SIZE; t++)
            {
#pragma HLS UNROLL
                const ap_uint<2> base = fwd.range(2*t+1, 2*t);
                rev.range(2*(SMER_SIZE-1-t)+1, 2*(SMER_SIZE-1-t)) =
                    base ^ ap_uint<2>(0x2);
            }
...
#pragma HLS INLINE off
    ...
    constexpr int buff_values  = PAR_FACTOR - n_last_round;
    const ap_uint<PAR_FACTOR> last_valid =
        ((ap_uint<PAR_FACTOR>)1 << n_last_round) - 1;
...
#pragma HLS loop_tripcount min=100 max=100
        in_word  = smer_stream_i.read();
        in_valid = smer_valid_i.read();
        const ap_uint<buff_values*S_BITS>  up_part_d =
            in_word.range(S_BITS*PAR_FACTOR-1, S_BITS*n_last_round);
        const ap_uint<buff_values>         up_part_v =
            in_valid.range(PAR_FACTOR-1, n_last_round);
        const ap_uint<n_last_round*S_BITS> dw_part_d =
            in_word.range(S_BITS*n_last_round-1, 0);
        const ap_uint<n_last_round>        dw_part_v =
            in_valid.range(n_last_round-1, 0);
...
#pragma HLS UNROLL
            ...
            for (int w = 1; w < WINDOW_SIZE; w++) {
                const ap_uint<S_BITS> v =
                    win.range((i+w+1)*S_BITS-1, (i+w)*S_BITS);
                if (v < m) m = v;
            }

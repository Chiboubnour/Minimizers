...
#pragma HLS loop_tripcount min=100 max=100
        ...
        if (v == 0) {
            // Le mot precedent a laisse jusqu'a PAR_FACTOR-offset elements
            // valides en attente dans buffer_d/buffer_v (realignement par
            // decalage de "offset"). Sans ce flush, ils seraient perdus car
            // aucun mot suivant n'arrive plus pour completer la combinaison.
            if constexpr (offset != 0) {
                if (buffer_v != 0) {
...
#pragma HLS loop_tripcount min=100 max=100
        ...
        if (v == 0) {
            // Meme flush que dans m3_thr_base_adapter : le dernier mot reel
            // a laisse des elements en attente dans buffer_d/buffer_v.
            if constexpr (offset != 0) {
                if (buffer_v != 0) {
...
//
/////////////////////////////////////////////////////////////////////////////////
// thr_axis_pack : combine (mot compacte, compteur) issus de thr_compact en
// une seule transaction de AXIS_BITS bits -- c'est la frontiere AXI4-Stream
// vers le kernel "store" (IP separe, frequence differente). Empaqueter les
// deux champs ensemble evite d'avoir a garder deux flux independants
// synchronises element-par-element a travers deux clock converters distincts
// cote Vivado.
//
template<int PAR_FACTOR>
void thr_axis_pack(

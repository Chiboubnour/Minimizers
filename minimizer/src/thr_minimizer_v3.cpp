...
#pragma HLS UNROLL
            ...
            if (!initialized) {
                init_count++;
                if (init_count == SMER-1) initialized = true;
                continue;
            }
...
#pragma HLS INTERFACE s_axilite port=return
    ...
    hls::stream<ap_uint<64>> base_stream;
    hls::stream<ap_uint<8>>  base_valid;
    hls::stream<ap_uint<8*SMER>> smer_stream;
    hls::stream<ap_uint<8>> smer_valid;
    hls::stream<ap_uint<8*SMER>> dedup_stream;
    hls::stream<ap_uint<8>> dedup_valid;
...
void minimizer_v3(
    ...
    if (s != 19 || w != 16) {
        fprintf(stderr, "[ERROR] minimizer_v3 supports only s=19, w=16\n");
        *nMinizrs = 0;
        return;
    }

    ap_uint<512>* tab_hash_ap = reinterpret_cast<ap_uint<512>*>(tab_hash);
    ap_uint<64> nMin = 0;

    minimizer(seq, tab_hash_ap, &nMin, n);

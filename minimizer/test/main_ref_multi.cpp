...
{
    return t_minimizer_sequence_omp<19,16>(
        sequence,
        n_bases,
        tab_hash,
        n_threads);
...
int main(int argc, char* argv[])
{
    ...
    // -------------------------
    std::string i_file;
    std::string o_file = "result_omp.txt";
    ...
    // -------------------------
    // Séquence (un seul buffer continu, pas de découpage en blocs) :
    // le découpage inter-threads est déjà géré avec chevauchement à
    // l'intérieur de t_minimizer_sequence_omp (voir ref_minimizer_multi.cpp),
    // donc il ne faut PAS re-découper la séquence ici en amont.
    // -------------------------

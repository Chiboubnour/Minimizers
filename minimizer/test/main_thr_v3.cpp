...
int main(int argc, char * argv[])
{
    ...
    for (int i = 0; i < n_words; i++) {
        ...
        for (int j = 0; j < 8; j++) {
            int idx = i*8 + j;
            if (idx < n) {
                uint8_t nuc = nucl[idx]; // ASCII character
                word |= ((uint64_t)nuc) << (8*j);
            }
        }
        seq64[i] = word;
    }
    ...
    // ----------------------------------
    int tab_size = (n_smers + 7) / 8; // 8 s-mers par mot de 512 bits
    ...
    uint64_t total = nMinizrs;
    printf("\n✅ REAL number of minimizers found: %" PRIu64 "\n\n", total);

    // ----------------------------------
    // Print first/last minimizers
    // ----------------------------------
    if (verbose && total > 0) {
        const int show = 5;
    ...
    if (ff != nullptr) {
        ...
        for (uint64_t i = 0; i < total; ++i) {
            uint64_t val = (uint64_t)tab_hash[i/8].range((i%8+1)*64-1, (i%8)*64);
            fprintf(ff, "s-mer [%6" PRIu64 "] : hash 0x%016" PRIx64 "\n", i, val);
        }
        fclose(ff);

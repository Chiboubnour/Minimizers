//
//  Testbench unitaire du module m5 (hash), template sur PAR_FACTOR.
//
//  Chaine m1<PF> -> ... -> m5<PF>. Sequence aplatie des hashes valides ==
...
static std::vector<uint64_t> run_flat(uint64_t n_bases)
{
    ...
    while (!s_hash.empty()) {
        const ap_uint<2 * SMER_SIZE * PF> w = s_hash.read();
        const ap_uint<PF>                 v = s_hash_v.read();
...
/////////////////////////////////////////////////////////////////////////////////
//
TEST_CASE("m5 hash PF=8",  "[m5][hash]") { const uint64_t n = GENERATE(64,63,62,61,60,59,58,57); check<8> (n); }
TEST_CASE("m5 hash PF=16", "[m5][hash]") { const uint64_t n = GENERATE(64,63,62,61,60,59,58,57); check<16>(n); }
TEST_CASE("m5 hash PF=32", "[m5][hash]") { const uint64_t n = GENERATE(64,63,62,61,60,59,58,57); check<32>(n); }
TEST_CASE("m5 hash PF=64", "[m5][hash]") { const uint64_t n = GENERATE(64,63,62,61,60,59,58,57); check<64>(n); }
//
//
/////////////////////////////////////////////////////////////////////////////////
//

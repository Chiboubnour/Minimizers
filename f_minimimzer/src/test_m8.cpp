//
//  Testbench unitaire du module m8 (dedup), template sur PAR_FACTOR.
//
//  Chaine m1<PF> -> ... -> m8<PF>. Sequence aplatie des minimizers valides ==
//  deduplication consecutive de la sequence des minima de m7. PF = 8,16,32,64.
//
#include <cstdint>
#include <cstdio>
#include <string>
...
//  minimizers attendus = dedup consecutif des minima
static std::vector<uint64_t> expected_dedup(uint64_t n_bases)
{
...
static std::vector<uint64_t> run_flat(uint64_t n_bases)
{
    ...
    while (!s_ded.empty()) {
        const ap_uint<2 * SMER_SIZE * PF> w = s_ded.read();
        const ap_uint<PF>                 v = s_ded_v.read();
...
static void check(uint64_t n_bases)
{
    ...
    const std::vector<uint64_t> exp = expected_dedup(n_bases);
    const std::vector<uint64_t> got = run_flat<PF>(n_bases);
    REQUIRE( got.size() == exp.size() );
    for (size_t j = 0; j < exp.size(); j++) { INFO("minimizer index j = " << j); REQUIRE( got[j] == exp[j] ); }
}
//
//
/////////////////////////////////////////////////////////////////////////////////
//
TEST_CASE("m8 dedup PF=8",  "[m8][dedup]") { const uint64_t n = GENERATE(64,63,62,61,60,59,58,57); check<8> (n); }
TEST_CASE("m8 dedup PF=16", "[m8][dedup]") { const uint64_t n = GENERATE(64,63,62,61,60,59,58,57); check<16>(n); }
TEST_CASE("m8 dedup PF=32", "[m8][dedup]") { const uint64_t n = GENERATE(64,63,62,61,60,59,58,57); check<32>(n); }
TEST_CASE("m8 dedup PF=64", "[m8][dedup]") { const uint64_t n = GENERATE(64,63,62,61,60,59,58,57); check<64>(n); }
//
//
/////////////////////////////////////////////////////////////////////////////////
//

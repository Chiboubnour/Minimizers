//
//  Testbench unitaire du module m7 (minimum glissant), template sur PAR_FACTOR.
//
//  Chaine m1<PF> -> ... -> m7<PF>. Sequence aplatie des minima valides ==
//  min des hashes [t-(WINDOW_SIZE-1) .. t], pour t = WINDOW_SIZE-1 .. H-1,
//  ou H = n_bases-(SMER_SIZE-1) et h_j = ref_hash(ref_smer(SMER_SIZE-1+j)).
//  PF = 8, 16, 32, 64.
//
#include <cstdint>
#include <cstdio>
...
static uint64_t ref_value(int j) { return ref_hash(ref_smer((SMER_SIZE - 1) + j)); }   // h_j
static uint64_t ref_min(int t)   // min des hashes [t-(W-1) .. t]
{
...
static std::vector<uint64_t> run_flat(uint64_t n_bases)
{
    ...
    while (!s_min.empty()) {
        const ap_uint<2 * SMER_SIZE * PF> w = s_min.read();
        const ap_uint<PF>                 v = s_min_v.read();
...
static void check(uint64_t n_bases)
{
    ...
    for (int t = WINDOW_SIZE - 1; t < H; t++) exp.push_back(ref_min(t));
    const std::vector<uint64_t> got = run_flat<PF>(n_bases);
    REQUIRE( got.size() == exp.size() );
    for (size_t j = 0; j < exp.size(); j++) { INFO("min index j = " << j); REQUIRE( got[j] == exp[j] ); }
}
//
//
/////////////////////////////////////////////////////////////////////////////////
//
TEST_CASE("m7 min glissant PF=8",  "[m7][min]") { const uint64_t n = GENERATE(64,63,62,61,60,59,58,57); check<8> (n); }
TEST_CASE("m7 min glissant PF=16", "[m7][min]") { const uint64_t n = GENERATE(64,63,62,61,60,59,58,57); check<16>(n); }
TEST_CASE("m7 min glissant PF=32", "[m7][min]") { const uint64_t n = GENERATE(64,63,62,61,60,59,58,57); check<32>(n); }
TEST_CASE("m7 min glissant PF=64", "[m7][min]") { const uint64_t n = GENERATE(64,63,62,61,60,59,58,57); check<64>(n); }
//
//
/////////////////////////////////////////////////////////////////////////////////
//
